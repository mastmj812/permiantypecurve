"""Selection endpoints — turn a drawn polygon / bbox / api14 list into a
...
class SummaryRequest(BaseModel):
    api14s: list[str] = Field(min_length=0, max_length=HARD_SELECTION_CAP + 1)
...
def _rows_for_api14s(session: Session, api14s: list[str]) -> list[WellSummaryRow]:
    if not api14s:
        return []
    res = session.execute(
        select(
...
def select_wells(
    ...
    # Hard-cap behavior: surface ALL matching api14s up to cap+1 so the
    # client can show "513 wells selected — reduce to ≤ 500".
    api14s = [r.api14 for r in summary_rows]
