...
def _load_monthly(session: Session, api14: str) -> pd.DataFrame:
...
def _persist(
    ...
    *,
    ...
    """Upsert (api14, stream) → forecasts. Honors `locked` if a prior row
    exists — locked forecasts are skipped so a bulk re-fit doesn't wipe
    a user's manual override."""
    existing = session.execute(
        select(Forecast).where(
            Forecast.api14 == api14, Forecast.stream == Stream(stream)
        )
    ).scalar_one_or_none()
    if existing is not None and existing.locked:
        log.info("forecast_locked_skip", api14=api14, stream=stream)
        return existing.id
    ...
    update_cols = {
        c: stmt.excluded[c]
        for c in values
        if c not in {"id", "api14", "stream", "created_at", "manual_override", "locked"}
    }
    stmt = stmt.on_conflict_do_update(
        constraint="uq_forecasts_api14_stream", set_=update_cols
    )
...
def forecast_well(
    ...
    *,
    ...
    monthly = _load_monthly(session, api14)
    out: dict[str, ForecastResult | None] = {"oil": None, "gas": None, "water": None}
    if monthly.empty:
        log.warning("no_production", api14=api14)
        return out
    ...
    for stream in STREAMS:
        ...
        try:
            ...
            log.exception("fit_failed", api14=api14, stream=stream, err=str(e))
            continue
        ...
        if stream != "oil":
            stream_peak_rate = _stream_rate_at_peak(monthly, oil_peak, stream)
            result = replace(result, peak_rate=stream_peak_rate)

        out[stream] = result
        if persist:
            _persist(session, api14=api14, stream=stream, result=result)

    return out


def _stream_rate_at_peak(monthly: pd.DataFrame, oil_peak: PeakResult, stream: str) -> float:
    col = {
    ...
    if oil_peak.peak_index >= len(df):
        return 0.0
    val = df.iloc[oil_peak.peak_index][col]
...
def forecast_wells(
    ...
    *,
    ...
    for api14 in api14s:
        results[api14] = forecast_well(session, api14, config=config)
    return results
