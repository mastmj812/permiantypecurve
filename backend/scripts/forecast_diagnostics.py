...
def _holdout_bias(monthly, peak, stream, cfg) -> float | None:
    ...
    pred = evaluate_well_rate(
        qo=r.qi, peak_index_months=None, qi=r.qi, Di=r.di_initial,
        b=r.b if r.b is not None else 1.0, Df=r.df_terminal or 0.08, t_years=t_test,
    )
...
def diagnose(
    tc_name: str, streams: tuple[str, ...], cfg: ForecastConfig | None = None
) -> None:
    ...
    with SessionLocal() as s:
        ...
        for api10 in api10s:
            ...
            for st in streams:
                ...
                if actual_cum > 0 and len(sl):
                    pred = evaluate_well_rate(
                        qo=r.qi, peak_index_months=None, qi=r.qi, Di=r.di_initial,
                        b=r.b or 1.0, Df=r.df_terminal or 0.08,
                        t_years=sl["t_years"].to_numpy(dtype=float),
                    )
                    model_cum = float(np.trapezoid(pred, sl["t_years"].to_numpy(dtype=float)) * 365.0)
        ...
        for st in streams:
            ...
            print(f"  qi_capture: p25={pct(qc,25):.2f} p50={pct(qc,50):.2f} p75={pct(qc,75):.2f}"
                  f" | share<0.80: {100*sum(v<0.8 for v in qc)//n}%")
            print(f"  yr1 decline fit p50={pct([x['de_fit'] for x in rs],50)*100:.0f}%"
                  f" empirical p50={pct([x['de_emp'] for x in rs],50)*100:.0f}%"
                  f" | shallow>10pp: {100*sum(g<-0.10 for g in gaps)//max(len(gaps),1)}%")
            print(f"  cum-to-date p50={pct([x['cum_ratio'] for x in rs],50):.2f}"
            ...
            for label, grp in (("qi<0.80", low), ("qi>=0.80", ok)):
                ...
                print(f"    {label} n={len(grp):>2}: di_gap p50={med(g)*100:+.0f}pp"
            ...
            if len(pairs) > 5:
                ...
                print(f"    corr(qi_capture, di_gap) = {c:+.2f}"
